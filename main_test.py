    ...
    if args.predict_mode == True and args.model_path is None:
        ...
        while True:
            ...
            if language not in ['k', 'e', 'f', 'j']:
                ...
                while True:
                    input('商品を指定の位置に設置し「Enter」を押して下さい')
                    ...
                    if args.predict_mode == 'False':
                        ...
                        print('商品を読み取るまで {}[s] 経過しました'.format(time_end-time_start))
                        print('商品 : not predict が読み取られました')
                        ...
                        if key == 'y':
                            ...
                            while True:
                                key = input('続けて商品をスキャンする場合は「y」,終了場合は「f」を押して下さい')
                                if key != 'y' and key != 'f':
                                    print('正しく入力されていません')
                                else:
                                    break
